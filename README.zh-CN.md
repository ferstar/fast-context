...
- **零中转、无遥测**：Fast Context **没有搭建任何中间代理服务器或中转后端，也不收集任何使用数据与埋点**。所有代码完全开源、本地执行。
- **直连官方服务**：在混合/远端模式（`--backend hybrid` / `remote`）下，程序仅使用用户本机已有的凭据，直接与 Windsurf / Devin 官方 API 通信。数据的处理与保留完全遵循你与 Windsurf / Devin 签署的服务条款及隐私政策，与本项目无关。
- **完全离线的纯本地模式（`--backend local`）**：
  - 索引与检索完全在本地运行（基于 Semble），**不会产生任何外部网络请求**。所有代码分块与缓存均保存在本地磁盘，适用于内网隔离环境或对三方服务严格禁用的合规场景。
- **本地防护措施**：
  - **路径脱敏**：本地实际绝对路径（如 `/Users/.../project`）在发送给官方接口前会被统一重映射为虚拟路径 `/codebase`，避免泄露开发者的本地用户名和目录结构。
  - **不上传全仓代码**：不会打包上传代码库，仅在交互轮次中将查询词、脱敏 Repo Map、词法线索，以及官方模型按需下发 `rg` / `readfile` 命令时读取的代码片段（均有行数与宽度截断保护）发给接口。
  - **本地沙箱限制**：所有由远端下发的只读命令均在本地严格执行边界校验，严禁越界访问项目根目录之外的文件。
